...
"""
    20160914 : DEW
    This file has been modified so the softmax classifier does not alter the 
    the input.  In other words the backprop method no longer transposes X.  This
    also means that the bias unit must be added before it is passed to the Softmax
    object.  The bias must also be added before passing the X to the predict method.
    These bias modifications also affect the base NeuralNet object constructor.
"""
"""
...
class NeuralNet(object):
    ...
    def __init__(self, input, targets, architecture={}, LAMBDA=0.0, ALPHA=0.0, classify=False, maxiter=100, saveFile=None):
        ...
        assert ALPHA >= 0, \
               "<ALPHA> must be a positive number: %r is not valid\n" % ALPHA
...
class SoftMaxClassifier(NeuralNet):
    ...
    def feedForward(self, theta, input):
        #print np.shape(theta), np.shape(input)
    ...
    def backProp(self, theta, hypothesis, input, indicatorFunction, m):
        ...
        #            + self._LAMBDA*theta
        thetaGrad = (1/m*(thetaGrad + np.dot(input, (hypothesis - indicatorFunction).T).T)) \
                    + self._LAMBDA*theta
    ...
    def costFunction(self, params, input, indicatorFunction):
        ...
        cost = 0
        regTerm = 0
        m = float(np.shape(input)[1]) # input should be (n+1)xm # Cast as float 6/11/14 to pass gradient check
        ...
        regTerm += (self._LAMBDA/2.0 * np.sum(np.multiply(theta, theta)))
...
class SoftMaxOnline(SoftMaxClassifier):
    """
        object:     SoftMaxOnline
        superclass: SoftMaxClassifier
    """
    def __init__(self, input, targets, LAMBDA=0.0, ALPHA=0.1, saveFile=None):
        architecture = {}
        # for online learning, maxiter should be the number of input examples, as we
        # wish to use each example individually to update the weights.
        maxiter = np.shape(input)[1]
        NeuralNet.__init__(self, input, targets, architecture, LAMBDA, ALPHA, True, maxiter, saveFile)
        self._indicatorFunction = self.labelsToIndicatorFunction(self._targets)
        self._architecture = {0:np.shape(self._input)[0]-1, 1:np.shape(self._indicatorFunction)[0]}
    ...
    def train(self):
        ...
        args = self._input, self._indicatorFunction
        # do not want to reinitialise weights everytime
        #initialParams = self.initialise()
        try:
            ...
            assert self._trainedParams is not None
            initialParams = self._trainedParams
